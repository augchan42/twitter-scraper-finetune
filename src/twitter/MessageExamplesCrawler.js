...
class MessageExamplesCrawler {
  ...
  async addExample(pid, count = 40, rankingMode = "Relevance") {
    ...
    if (
      ...
      conversationList[0]["content"]["itemContent"]["tweet_results"]["result"][
        "legacy"
      ]["is_quote_status"]
    ) {
      isQuotedTweet = true;
      const quotedMessageExample = this.processQuotedTweet(conversationList[0]);
  ...
  processTweet(tweet) {
    ...
    for (let i = 0; i < tweetList.length; i++) {
      ...
      if (tweetList[i]["item"]["itemContent"]["cursorType"] === "ShowMore") {
        break;
      }
      try {
        const tweetText =
